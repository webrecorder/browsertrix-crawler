...
import { type TestConfig, isCI, testIf } from "./utils";

function runCrawl(name: string, config: TestConfig, commandExtra = "") {
...
function checkVideo(coll: string) {
...
testIf(
  !isCI,
  "test block rule based on iframe text, content included due to match",
  () => {
    const config = {
...
testIf(
  !isCI,
  "test rule based on iframe text not matching, plus allowOnly iframe",
  () => {
    const config = {
      url: "https://oembed.link/https://www.youtube.com/watch?v=aT-Up5Y4uRI",
      blockRules: [
...
testIf(
  !isCI,
  "test block rules complex example, block external urls on main frame, but not on youtube",
  () => {
    const config = {
