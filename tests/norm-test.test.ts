...
test("same URL with same query args, but different sort order", async () => {
  ...
  execSync(
    "docker run --rm -v $PWD/test-crawls:/crawls webrecorder/browsertrix-crawler crawl --url 'https://example-com.webrecorder.net/?B=1&A=2' --url 'https://example-com.webrecorder.net/?A=2&B=1' --collection norm-test-1",
  );

  const parser = loadFirstWARC("norm-test-1");

  const uris = ["https://example-com.webrecorder.net/?B=1&A=2"];
