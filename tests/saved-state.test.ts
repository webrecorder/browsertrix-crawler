...
import { SaveState } from "../src/util/state";

const pagesFile = "test-crawls/collections/int-state-test/pages/pages.jsonl";
const extraPagesFile =
  "test-crawls/collections/int-state-test/pages/extraPages.jsonl";

async function waitContainerDone(containerId: string) {
...
let savedStateFile: string;
let state: SaveState;
let numDone: number;
let numQueued: number;
let finished: string[];
...
test("check crawl interrupted + saved state written", async () => {
  ...
  try {
    containerId = execSync(
      'docker run -d -v $PWD/test-crawls:/crawls -v $PWD/tests/fixtures:/tests/fixtures webrecorder/browsertrix-crawler crawl --collection int-state-test --url http://old.webrecorder.net/ --limit 10 --behaviors "" --exclude community',
      { encoding: "utf-8" },
      //wait.callback,
    );
  } catch (error) {
  ...
  const savedStates = fs
    .readdirSync("test-crawls/collections/int-state-test/crawls")
...
test("check parsing saved state + page done + queue present", () => {
  ...
  const saved = yaml.load(savedState) as { state: SaveState };
...
test("check crawl restarted with saved state", async () => {
  ...
  const redis = new Redis(`redis://127.0.0.1:${port}/0`, {
    lazyConnect: true,
    retryStrategy: () => null,
  });
