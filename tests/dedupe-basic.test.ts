...
beforeAll(() => {
  ...
  redisId = execSync(
    "docker run --rm --network=dedupe -p 37379:6379 --name dedupe-redis -d redis",
  );
});
...
function deleteFirstWARC(name: string) {
  const archiveWarcLists = fs.readdirSync(
    `test-crawls/collections/${name}/archive`,
  );
...
function loadDataPackageRelated(name: string) {
  execSync(
...
test("import dupe index from json, reverse, revisits than orig, from wacz", async () => {
  ...
  fs.writeFileSync(
    "./test-crawls/collections/dedupe-test-dupe/import-1.json",
