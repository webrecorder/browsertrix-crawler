...
test("check that invalid URL in seed list fails crawl", async () => {
  let status = 0;

  try {
    await exec(
      "docker run -v $PWD/test-crawls:/crawls -v $PWD/tests/fixtures:/tests/fixtures webrecorder/browsertrix-crawler crawl --collection invalidfilelisttest --urlFile /tests/fixtures/urlSeedFileInvalid.txt --timeout 90000",
    );
  } catch (e) {
    status = e.status;
  }
