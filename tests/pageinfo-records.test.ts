...
test("run warc and ensure pageinfo records contain the correct resources", async () => {
  ...
  for await (const record of parser) {
    if (
      ...
      (record.warcTargetURI === "https://old.webrecorder.net/" ||
        record.warcTargetURI === "https://old.webrecorder.net/about")
    ) {
      expect(record.warcHeaders.headers.get("WARC-Protocol")).toBe(
        "h2, tls/1.3",
      );
    }
...
function validateResourcesIndex(json: Record<string, unknown>) {
...
function validateResourcesAbout(json: Record<string, unknown>) {
  expect(json).toHaveProperty("pageid");
  expect(json).toHaveProperty("url");
  expect(json).toHaveProperty("ts");
  expect(json).toHaveProperty("urls");
...
function validateResourcesInvalid(json: Record<string, unknown>) {
  expect(json).toHaveProperty("pageid");
  expect(json).toHaveProperty("url");
  expect(json).toHaveProperty("urls");
  expect(json.counts).toEqual({ jsErrors: 0 });
