...
import { exec as execCallback, ExecException, execSync } from "child_process";
...
test("ensure crawl fails if failOnFailedLimit is reached", async () => {
  ...
  try {
    ...
    expect((error as ExecException).code).toEqual(12);
    passed = false;
  }
...
test("ensure crawl fails if timeLimit is reached", async () => {
  ...
  try {
    ...
    expect((error as ExecException).code).toEqual(15);
    passed = false;
  }
  expect(passed).toBe(false);
});
...
test("ensure crawl fails if sizeLimit is reached", async () => {
  ...
  try {
    ...
    expect((error as ExecException).code).toEqual(14);
    passed = false;
  }
  expect(passed).toBe(false);
});
...
test("ensure crawl fails if diskUtilizationLimit is reached", async () => {
  ...
  try {
    ...
    expect((error as ExecException).code).toEqual(16);
    passed = false;
  }
  expect(passed).toBe(false);
});
