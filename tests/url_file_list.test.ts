...
beforeAll(() => {
  ...
  proc = spawn("../../node_modules/.bin/http-server", ["-p", "31502"], {
    cwd: fixtures,
  });
});
...
function verifyAllSeedsCrawled(collName: string, hasDownload: boolean) {
  const crawled_pages = fs.readFileSync(
    `test-crawls/collections/${collName}/pages/pages.jsonl`,
    "utf8",
  );

  const seedFile = hasDownload
    ? `test-crawls/collections/${collName}/downloads/seeds-seedFileCopy.txt`
    : "tests/fixtures/urlSeedFile.txt";
  const seed_file = fs.readFileSync(seedFile, "utf8").split("\n").sort();

  const seed_file_list = [];
  for (let j = 0; j < seed_file.length; j++) {
  ...
  for (let i = 1; i < seed_file_list.length; i++) {
...
test("start crawl, interrupt, stop seed file server, and ensure all seed URLs are crawled", async () => {
  ...
  // server no longer up
  await expect(() => fetch("http://localhost:31502/")).rejects.toThrow(
    "fetch failed",
  );
