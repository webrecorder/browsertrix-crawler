...
beforeAll(() => {
  ...
  storageId = execSync(
    `docker run --rm -d -p 39900:9000 --name s3storage --network=upload-test-net versity/versitygw --port :9000 --access ${ACCESS} --secret ${SECRET} posix /tmp/`,
    { encoding: "utf-8" },
  );
});
...
test("run crawl with upload", async () => {
  ...
  const redis = new Redis("redis://127.0.0.1:36390/0", {
    lazyConnect: true,
    retryStrategy: () => null,
  });
  ...
  const resp = await fetch(
    `http://127.0.0.1:39900/${BUCKET_NAME}/prefix/${filename}`,
  );
