...
When crawling, it is often possible to encounter rate limits, CAPTCHA or auth pages that are shown instead of valid content that crawler is intending to capture.
These 'auth-walled' pages are often done for a good reason, and bypassing such limits if outside of the scope of the crawler.
...
The simplest mechanism for detection is by checking the status code. By default, the crawler now skips all pages that have a 403, 429, 503 status codes. This list can be customized with the `--rateLimitStatusCodes` which can be set to a list of status codes to always skip and retry.

The crawler also includes a custom option, `--rateLimitOnMatch` which can take one more values in the form of
`<regex>` or `<regex>:<status>`, for example, `--rateLimitOnMatch <regex-1> --rateLimitOnMatch <regex-2>:<status>`.

This setting allows for custom detection of CAPTCHA / rate-limited pages based on page content, and optionally status code,
so even 200 pages can be flagged as rate limited.
...
If a site provides a `Retry-After` header, for example, with a 429 response, the timeout is used instead of the global default set in `--rateLimitTimeout`.

## Rate Limit Retry Count

By default, rate limited pages are retried indefinitely, allowing the crawl to possibly complete, but at a much slower pace. If the pages are blocked do to auth requirements, it may be possible to add a browser profile and restart the crawl, allowing for previously flagged pages to be captured successfully.

However, it may be desirable to set a total number of retries for pages flagged as rate limited, which can be done
with `--rateLimitMaxRetries` to a value >= 0, where 0 implies no retries at all.
...
This can allow another application (eg. Kubernetes) to provide an exponential backoff system when the crawler is repeatedly exiting due to rate limited pages.
...
If a page loads successfully, the rate limit counter is also cleared.

## Caveats and multi-domain crawling

The rate limit interrupt is disabled by default, as it may not always be desirable for crawling many seeds across different domains crawling, where one site may be rate limited while others' are not. Currently, the rate limits are applied globally and not per-domain.
...
If facing repeated errors, requesting permission may be the best approach to successful crawl a domain.