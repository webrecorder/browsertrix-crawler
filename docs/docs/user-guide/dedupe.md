...
The crawler also supports content-based deduplication by content across multiple crawls, with an external Redis (or Redis-compatible database) index used to stores the deduplication data.

To enable, add `--redisDedupeUrl <redis url>`, eg. with a standard Redis format URL: `redis://host:port/db`, eg. `--redisDedupeUrl redis://my-dedupe-index:6379/0`
...
been archived, it will not be saved again, instead a duplicate or [`revisit` record will be created](#warc) instead.
...
### Example: Running crawls with dedupe

Here's quick example of running crawls with dedupe against the same index:

1) Start Redis server, eg. `redis-server -p 10000`

2) Run a crawl with `--redisDedupeUrl` set, for example, a crawl of 5 pages from `https://old.webrecorder.net/`
...
## Create dedupe index from existing crawls

It may sometimes be useful to populate the dedupe index db with existing data, eg. one or more WACZ files of previous crawls. This can be done by reading through all the entries in the CDXJ index inside of a WACZ and populating the dedupe index based on this data.
...
By default, the `indexer` operation is additive only, crawl data is added, but not removed.
To remove all crawls that aren't part of `--sourceUrl` (either single WACZ or a JSON specifying multiple WACZs),
also add `--remove` flag. This will purge all data that is not being added.
...
See [WACZ dependency docs](../develop/dedupe.md#crawl-dependency-tracking-in-wacz-datapackagejson) for more details on the architecture of this dependency system.